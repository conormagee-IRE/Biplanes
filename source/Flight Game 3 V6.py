...
FPS = 60 if IS_WEB else BASE_FPS
DT = 1 / FPS
...
async def get_frame_time(previous_ticks):
    if IS_WEB:
        ...
        if 0.0 <= frame_time < DT:
            await asyncio.sleep(DT - frame_time)
            current_ticks = pygame.time.get_ticks()
            frame_time = (current_ticks - previous_ticks) / 1000.0
...
def build_lightning_overlay(lightning_points):
    int_points = [(int(x), int(y)) for x, y in lightning_points]
    if not int_points:
        return None, None

    glow_padding = 10
    min_x = min(x for x, _ in int_points) - glow_padding
    min_y = min(y for _, y in int_points) - glow_padding
    max_x = max(x for x, _ in int_points) + glow_padding
    max_y = max(y for _, y in int_points) + glow_padding
    overlay_rect = pygame.Rect(min_x, min_y, max(1, max_x - min_x + 1), max(1, max_y - min_y + 1))
    overlay_surface = pygame.Surface(overlay_rect.size, pygame.SRCALPHA)
    local_points = [(x - overlay_rect.x, y - overlay_rect.y) for x, y in int_points]
    pygame.draw.lines(overlay_surface, (255, 255, 210, 255), False, local_points, 8)
    pygame.draw.lines(overlay_surface, (255, 248, 170, 255), False, local_points, 4)
    return overlay_surface, overlay_rect
...
def update_storm_cloud(storm_cloud, dt):
    ...
    if storm_cloud["lightning_duration"] > 0.0:
        ...
        if storm_cloud["lightning_duration"] == 0.0:
            ...
            storm_cloud["lightning_surface"] = None
            storm_cloud["lightning_rect"] = None
    else:
        ...
        if storm_cloud["lightning_timer"] <= 0.0:
            ...
            lightning_surface, lightning_rect = build_lightning_overlay(storm_cloud["lightning_points"])
            storm_cloud["lightning_surface"] = lightning_surface
            storm_cloud["lightning_rect"] = lightning_rect
...
def draw_storm_cloud(surface, storm_cloud, fade_progress=1.0):
    ...
    lightning_surface = storm_cloud.get("lightning_surface")
    lightning_rect = storm_cloud.get("lightning_rect")
    if lightning_surface is None or lightning_rect is None:
        lightning_surface, lightning_rect = build_lightning_overlay(storm_cloud["lightning_points"])
        storm_cloud["lightning_surface"] = lightning_surface
        storm_cloud["lightning_rect"] = lightning_rect
    if lightning_surface is None or lightning_rect is None:
    ...
    surface.blit(get_alpha_variant(lightning_surface, glow_alpha), lightning_rect.topleft)
